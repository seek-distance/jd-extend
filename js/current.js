...
var Upload = function() {
    this.imgFirstUrl = 'http://jddr-img.jymao.com/';
...
AddToshop.prototype = {
    ...
    dealShop: function() {
        ...
        var imgUrl = self.message.fifthImg;

        var imgItem = '<div class="file_view"><img src="' + imgUrl + '" alt="" style="display: inline;"></div>';
        if ($('.file_view').length == 4) {
            $('.pd_img_list').append(imgItem);
        } else {
            $('.file_view').last().find('img').attr('src', imgUrl).css('display', 'inline');
        }
